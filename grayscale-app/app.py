...
def publish_notification(original_filename, operation):
    message = {
        ...
        'operation': operation,
        'body': f"O arquivo {original_filename} foi {operation}."
    }
...
def create_grayscale(path_file, original_filename):
    ...
    try:
        ...
        publish_notification(original_filename, 'transformado em preto e branco')
        logging.info(f"Imagem {original_filename} transformada para grayscale e salva em {output_path}")
        ...
        while True:
            ...
            if msg is None:
                ...
                try:
                    ...
                    if filename:
                        logging.warning(f"GRAYSCALE: Lendo {filename}")
                        create_grayscale(os.path.join(IN_FOLDER, filename), filename)
                        logging.warning(f"GRAYSCALE: Finalizado {filename}")
                    else:
